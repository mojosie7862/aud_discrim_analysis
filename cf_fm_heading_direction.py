...
xy_file = 'xy_results_2023-03-01_16.18.csv'
transcripts = 'combined_transcripts_2022-11-02.csv'
    ...
    if paradigm == 'cf' or paradigm == 'fm':
        ...
        for i_f in xy_raw_data.index: #over 200,000 iterations
            ...
            if frame_filename[8:-4] == video_filename[0:-4]:
                ...
                if xy_raw_data['frame'][i_f] < critical_frames['tone_start_frame']:
                    critical_data[v_zf_id][run]['pre_tone_xy'].append((xy_raw_data['HeadX'][i_f], xy_raw_data['HeadY'][i_f], xy_raw_data['frame'][i_f]))
                    critical_data[v_zf_id][run]['pre_tone_heading'].append(xy_raw_data['Heading'][i_f])
...
for id, videos_data in critical_data.items():
    transform_xy(videos_data, critical_data, 'pre_tone_xdata', 'pre_tone_ydata', 'pre_tone_zdata', 'pre_tone_xy')
...
for id in critical_data.keys():
    for r, data in critical_data[id].items():
        print(r)
        id_run = id + '_r' + r + '_' + data['paradigm']

        fig = plt.figure(figsize=(9, 6))
        ax = plt.axes(projection = '3d')
        ax.plot3D(data['pre_tone_xdata'], data['pre_tone_ydata'], data['pre_tone_zdata'], color = 'tab:gray')
